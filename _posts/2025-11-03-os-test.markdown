...
As we now have a nice baseline, we obviously want to improve our compliance. There are quite a few tests that are low-hanging fruit, and there are some features that -- if implemented -- would make entire classes of tests work (e.g. loopback interfaces).
...
As Managarm and mlibc have testsuites of their own already, we want to upstream them to os-test where reasonable, so other operating systems get to enjoy the benefits of a common test suite. Maybe your OS is the next to run os-test?
